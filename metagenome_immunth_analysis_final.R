##########07/01/24
#### Immunotherapy  project - metagenomics analysis  
...
######## KM curve of itme to progression / follow up exam #######


#use data - sample_info 
sample_info <- table_1_immun
...
survival.data <- sample_info[, c("TTP","ultimate_response_2_lev")]
survival.data$prgoression <- ifelse(survival.data$ultimate_response_2_lev=="PD", "1", "0")

#rename the new df 
colnames(survival.data) <- c("time", "Progression", "status")
...
survival.data$Progression <- as.factor(survival.data$Progression)
survival.data
#remove NA 
...
#create survival df
sfit <- survfit(Surv(time, status)~Progression, data=survival.data)
sfit

survival.plot <- ggsurvplot(sfit, data = survival.data,
                            fun = "pct", ggtheme = theme_pubr(), 
                            conf.int=FALSE, pval=TRUE, risk.table=TRUE, 
                            legend.labs=c("Non Porgressive", "Porgressive"), legend.title="Progression",  
                            palette=c("blue", "red"), 
                            size=1,
                            title="Kaplan-Meier Curve for Progression",
                            ylab="PFS",
                            xlab="Time(Days)",
...
#save it
write.csv(my.comparisons, file = "metagenomics/Results/ddPCR.stats.one_y_mort.csv")

ddpcr_dat <- ddpcr_dat %>% 
  mutate(one_y_mort= factor(one_y_mort, levels=c("greater_12", "less_12")))

# plot it
pdf(file = "metagenomics/Figures/ddPCR_one_y_mort.pdf", width=6, height=9)
...
#save it
write.csv(my.comparisons, file = "metagenomics/Results/Shannon.stats_one_y_mort.csv")

# plot it
pdf(file = "metagenomics/Figures/Shannon_one_y_mort.pdf", width=6, height=9)
...
#observed 
pdf(file = "metagenomics/Figures/observed_one_y_mort.pdf", width=6, height=9)
...
pdf(file = "metagenomics/Figures/Beta.Diversity.Bray.one_y_mort.pdf", 
    height = 20, width = 18)
ggplot(newResults, aes(PC1, PC2, color= one_y_mort)) + # Graph PC1 and PC2
  ...
  ggtitle("Beta Diversity, Bray, one_y_mort, p=0.071")+
  theme(panel.background = element_blank(),panel.border=element_rect(fill=NA),panel.grid.major = element_line(linetype = "dashed", size = 0.5, colour = "darkgrey"),panel.grid.minor = element_blank(),strip.background=element_blank(),axis.title=element_text(size=30,face="bold"),axis.text.x=element_text(colour = "darkgrey", size = rel(0.75)),axis.text.y=element_text(colour = "darkgrey", size = rel(0.75)),axis.ticks=element_blank(),plot.margin=unit(c(1,1,1,1),"line"), legend.position="none")
dev.off()

#sats
write.table(adonis2(vegdist ~ newResults$one_y_mort), 
            file = "metagenomics/Results/Beta.Diversity.Bray.one_y_mort.txt", sep = "\t", row.names = T)
...
# save results
write.csv(edgeR.to.save, file = "metagenomics/Results/edgeR.results_one_y_mort_high_vs_low.csv")
...
pdf(file="metagenomics/Figures/EdgeR_bubble_one_y_mort.pdf", height = 11, width = 14)
bubble.plot+
  guides(size="none")
dev.off()
...
my_legend <- get_legend(bubble.plot)
#save it 
pdf(file="metagenomics/Figures/EdgeR_bubble_one_y_mort_legend.pdf", 
    height = 4, width = 2)
as_ggplot(my_legend)
dev.off() 
...
# save results
write.csv(edgeR.to.save, file = "metagenomics/Results/edgeR.results_one_y_mort_adjusted_for_covariates.csv")
...
pdf(file="metagenomics/Figures/EdgeR_bubble_one_y_mort_adjusted_for_covariates.pdf", height = 11, width = 14)
bubble.plot+
  guides(size="none")
dev.off()
...
my_legend <- get_legend(bubble.plot)
#save it 
pdf(file="metagenomics/Figures/EdgeR_bubble_one_y_mort_adjusted_for_covariates_legend.pdf", 
    height = 4, width = 2)
as_ggplot(my_legend)
dev.off() 
...
#save it
write.csv(my.comparisons, file = "metagenomics/Results/ddPCR.stats.PD_L1_expression_2_lev.csv")

ddpcr_dat <- ddpcr_dat %>% 
  mutate(PD_L1_expression_2_lev= factor(PD_L1_expression_2_lev, levels=c("low_50", "high_50")))

# plot it
pdf(file = "metagenomics/Figures/ddPCR_PD_L1_expression_2_lev.pdf", width=6, height=9)
...
#save it
write.csv(my.comparisons, file = "metagenomics/Results/Shannon.stats_PD_L1_expression_2_lev.csv")

# plot it
pdf(file = "metagenomics/Figures/Shannon_PD_L1_expression_2_lev.pdf", width=6, height=9)
...
#observed 
pdf(file = "metagenomics/Figures/observed_PD_L1_expression_2_lev.pdf", width=6, height=9)
...
pdf(file = "metagenomics/Figures/Beta.Diversity.Bray.PD_L1_expression_2_lev.pdf", 
    height = 20, width = 18)
...
write.table(adonis2(vegdist ~ newResults$PD_L1_expression_2_lev), 
            file = "metagenomics/Results/Beta.Diversity.Bray.PD_L1_expression_2_lev.txt", sep = "\t", row.names = T)
...
# save results
write.csv(edgeR.to.save, file = "metagenomics/Results/edgeR.results_PD_L1_expression_2_lev_high_vs_low.csv")
...
pdf(file="metagenomics/Figures/EdgeR_bubble_PD_L1_expression_2_lev_high_vs_low.pdf", height = 11, width = 14)
bubble.plot+
  guides(size="none")
dev.off()
...
my_legend <- get_legend(bubble.plot)
#save it 
pdf(file="metagenomics/Figures/EdgeR_bubble_PD_L1_expression_2_lev_high_vs_low_legend.pdf", 
    height = 4, width = 2)
as_ggplot(my_legend)
dev.off() 
...
pdf(file = "metagenomics/Figures/Beta.Diversity.Bray.k_means_2_clusters_and_Sample_type_al_samples_unpruned.pdf", 
    height = 20, width = 18)
...
# plot it
pdf(file = "metagenomics/Figures/Shannon_NAT_k_means_2_cluster_all_samples.pdf", width=6, height=9)
...
pdf(file = "metagenomics/Figures/Shannon_NAT_k_means_2_cluster_all_samples_facet.pdf", width=11, height=9)
ggplot(alpha.measures, aes(x=k_means_2_cluster, y=Shannon, fill=k_means_2_cluster))+
  geom_boxplot(outlier.shape = NA)+
  stat_boxplot(geom = "errorbar", width= 0.15)+
  scale_fill_manual(values = c("#F8766D","#00bfc4"))+
...
survival.plot <- ggsurvplot(sfit, data = survival.data,
                            fun = "pct", ggtheme = theme_pubr(), 
                            conf.int=FALSE, pval=TRUE, risk.table=TRUE, 
                            legend.labs=c("Cluster 1", "Cluster 4"), legend.title="K means Cluster",  
                            #palette=c("brown2", "burlywood2", "chocolate2"), 
                            size=1,
                            title="Kaplan-Meier Curve for one year mortaltiy  According to K means Cluster",
                            ylab="Overall mortality Probability",
...
pdf(file = "metagenomics/Figures/Beta.Diversity.Bray.k_means_4_clusters_and_Sample_type_al_samples_unpruned.pdf", 
    height = 20, width = 18)
...
# plot it
pdf(file = "metagenomics/Figures/Shannon_NAT_k_means_4_cluster_all_samples.pdf", width=6, height=9)
...
#facet by sample type 
pdf(file = "metagenomics/Figures/Shannon_NAT_k_means_4_cluster_all_samples_facet.pdf", width=11, height=9)
ggplot(alpha.measures, aes(x=k_means_4_cluster, y=Shannon, fill=k_means_4_cluster))+
  geom_boxplot(outlier.shape = NA)+
  stat_boxplot(geom = "errorbar", width= 0.15)+
  #scale_fill_manual(values = c("#F8766D","#00bfc4"))+
...
#plot
pdf(file = "metagenomics/Figures/AUC_taxa_level_RF_one_y_mort.pdf", height = 6, width = 12 )
...
df2$cutoffs[which.min(df2$value)] #50

#save the AUC values 
write.csv(df2, file = "metagenomics/Results/AUC_values_one_y_mort.csv" )
...
median(AUCs$value)
pdf(file = "metagenomics/Figures/AUC_best_cutoff_10_Metagenome_RF_one_y_mort.pdf", height = 6, width = 3)
...
write.csv(VI_taxon, file="metagenomics/Results/RF_variable_importance_best_AUC_one_y_mort.csv")
...
write.csv(cox.results, file="metagenomics/Results/cox_results_Metagenome_one_y_mort.csv")



####### cox figure : no need in this case as only one contaminant is significnat 

#seperate CI 
cox.results <- cox.results %>%
  separate(CI, into = c("CI_lower", "CI_upper"), sep = "-", convert = TRUE)

plot_dat <- cox.results


#set color vector accroding to contam
plot_dat <- plot_dat %>% 
  mutate(contaminant= factor(contaminant)) %>% 
  mutate(color=ifelse(plot_dat$contaminant =="TRUE", "red", "black")) %>% 
  mutate(CI_upper= as.numeric(CI_upper)) %>% 
  mutate(CI_lower= as.numeric(CI_lower)) %>% 
  mutate(Hazard.ratio= as.numeric(Hazard.ratio)) %>% 
  mutate(taxa_colored = plot_dat$taxa) %>% 
  filter(adj.p<=0.2) %>% 
  arrange(.,desc(Hazard.ratio)) %>% 
  dplyr::mutate(ord = order(plot_dat$taxa))

#add color of potential contaminants 
plot_dat$taxa_colored <- paste0("<span style=\"color: ", plot_dat$color, "\">", plot_dat$taxa_colored, "</span>")

#choose top 40 
plot_dat_2 <- plot_dat %>% dplyr::slice(1:40)

#plot 
ggplot(data=plot_dat_2, aes(x=Hazard.ratio, y=fct_reorder(taxa_colored, -ord), xmin=CI_lower, xmax=CI_upper))+
  geom_point(size=5)+
...
#plot
pdf(file = "metagenomics/Figures/AUC_taxa_level_RF_two_y_mort.pdf", height = 6, width = 12 )
ggline(AUCs_taxon, x = "cutoffs", y = "value",  position = position_dodge(0.8), color = "dodgerblue",
       add = c("mean_sd"),numeric.x.axis = TRUE)+theme_bw(base_size = 14)+
  ylab("AUC")+xlab("% Top Taxa")+
  scale_x_continuous(breaks = seq(0,100, by = 10))+ 
...
#get best cutoff for AUC 
df2$cutoffs[which.max(df2$value)] #70
...
for (i in 1:20){
  ...
  ## top 70%
  taxa.top <- mdi$taxa[1:round(nrow(mdi)*0.7)]
...
AUCs=melt(AUCs, id.vars=1)
median(AUCs$value)
pdf(file = "metagenomics/Figures/AUC_best_cutoff_best_Metagenome_RF_two_y_mort.pdf", height = 6, width = 3)
ggboxplot(AUCs, x = "variable", y = "value", color="variable",
          add = "jitter")+theme_bw(base_size = 14)+
  ylab("AUC")+xlab("")+ggtitle("AUCs in the Metagenomecriptome dataset across 20 repetitions, median 0.71")+
  theme(legend.position = "none")
dev.off()
...
write.csv(VI_taxon, file="metagenomics/Results/RF_variable_importance_best_AUC_two_y_mort.csv")
...
write.csv(cox.results, file="metagenomics/Results/cox_results_Metagenome_two_y_mort.csv")
...
#plot
pdf(file = "metagenomics/Figures/AUC_taxa_level_RF_PD.pdf", height = 6, width = 12 )
ggline(AUCs_taxon, x = "cutoffs", y = "value",  position = position_dodge(0.8), color = "dodgerblue",
       add = c("mean_sd"),numeric.x.axis = TRUE)+theme_bw(base_size = 14)+
  ylab("AUC")+xlab("% Top Taxa")+
  scale_x_continuous(breaks = seq(0,100, by = 10))+ 
...
#get best cutoff for AUC 
df2$cutoffs[which.max(df2$value)] #3
...
for (i in 1:20){
  ...
  ## top 70%
  taxa.top <- mdi$taxa[1:round(nrow(mdi)*0.03)]
...
AUCs=melt(AUCs, id.vars=1)
median(AUCs$value)
pdf(file = "metagenomics/Figures/AUC_best_cutoff_best_Metagenome_RF_PD.pdf", height = 6, width = 3)
ggboxplot(AUCs, x = "variable", y = "value", color="variable",
          add = "jitter")+theme_bw(base_size = 14)+
  ylab("AUC")+xlab("")+ggtitle("AUCs in the Metagenomecriptome dataset across 20 repetitions, median 0.61")+
  theme(legend.position = "none")
dev.off()
...
write.csv(VI_taxon, file="metagenomics/Results/RF_variable_importance_best_AUC_PD.csv")
...
write.csv(cox.results, file="metagenomics/Results/cox_results_Metagenome_PD.csv")





##############cox results plot, significant only for PD outcome ########

#read data 
cox_df <- read.csv(file = "metagenomics/Results/cox_results_Metagenome_PD.csv")
...
p <- ggplot(data=plot_df, aes(x=Hazard.ratio, y=fct_reorder(taxa_colored, -ord), xmin=lower_CI, xmax=upper_CI))+
  ...
  theme(axis.text.y= element_markdown(face = "bold", size = 20), 
        ...
        axis.title.x=element_text(face = "bold", size = 24))
#save 
pdf(file = "metagenomics/Figures/cox_results_Metagenome_PD.pdf", height = 14, width = 10)
p
dev.off()
...
write.csv(temp, file = "metagenomics/metadata_for_metagemone.csv")
write.table(temp_1, file = "metagenomics/MG_counts_pruned.txt", sep = "\t", row.names = TRUE)


temp_3 <- read.csv(file = "metatranscriptomics/metadata_for_metatrans.csv")
rownames(temp_3) <- temp_3$X

setdiff(rownames(temp_3),rownames(temp))
table(rownames(temp_3)==rownames(temp))




# Create the contingency table
data <- matrix(c(6, 0, 38, 23, 3, 6), 
               nrow = 2, 
               byrow = TRUE,
               dimnames = list(c("PD", "Non_PD"), c("Cluster_1", "Cluster_2", "Cluster_3")))
...
data <- matrix(c(6, 0,   # Cluster 1
                 38, 23, # Cluster 2
                 3, 6),  # Cluster 3
               nrow = 3, byrow = TRUE)
rownames(data) <- c("Cluster_1", "Cluster_2", "Cluster_3")
colnames(data) <- c("PD", "Non_PD")

# Perform pairwise comparisons
# Cluster 1 vs 2
data_1_2 <- data[c(1, 2), ]
chi_1_2 <- chisq.test(data_1_2)

# Cluster 1 vs 3
data_1_3 <- data[c(1, 3), ]
chi_1_3 <- chisq.test(data_1_3)

# Cluster 2 vs 3
data_2_3 <- data[c(2, 3), ]
chi_2_3 <- chisq.test(data_2_3)
...
save.image(file= "immuno_meta_genome_final.RData")
